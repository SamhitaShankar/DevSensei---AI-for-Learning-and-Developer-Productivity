...
export function RepoView({ repo, onBack }: RepoViewProps) {
  ...
  useEffect(() => {
    ...
    async function fetchRoot() {
      setLoading(true);
      try {
        const res = await fetch(`/api/repo/${repo.owner}/${repo.name}?path=`);
        if (res.ok) {
        ...
        console.error(err);
        toast({ title: "Error", description: `Could not load repository files. ${err.message}`, variant: "destructive" });
        setTree([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleSelectFile = async (file: FileNode) => {
    if (file.type === 'file') {
      try {
        const res = await fetch(`/api/repo/${repo.owner}/${repo.name}?path=${encodeURIComponent(file.path)}`);
        if (res.ok) {
          const data = await res.json();
          const decodedContent = data.content ? atob(data.content.replace(/\\s/g, '')) : '';
        ...
        console.error(err);
        toast({ title: "Error", description: `Could not load file content. ${err.message}`, variant: "destructive" });
      }
    }
  };
  ...
  const handleFetchChildren = async (path: string): Promise<FileNode[]> => {
    try {
      const res = await fetch(`/api/repo/${repo.owner}/${repo.name}?path=${encodeURIComponent(path)}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleShowArchitecture = async () => {
    ...
    try {
      const res = await fetch(`/api/repo/${repo.owner}/${repo.name}?tree=true`);
      if (res.ok) {
        const fullItems = await res.json();
        const treeString = fullItems.map((item: {path:string}) => item.path).join('\\n');
      ...
      toast({ title: "Analysis Error", description: `Could not analyze repository architecture. ${err.message}`, variant: "destructive" });
      setArchitectureData(null); // Clear old data on failure
    } finally {
      setIsAnalyzingArchitecture(false);
    }
  };
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={onBack} className="text-muted-foreground hover:text-primary rounded-lg">
            Back to Dashboard
          </Button>
          <div className="h-6 w-[1px] bg-border mx-2" />
          <h2 className="text-xl font-bold text-primary">{repo.name}</h2>
          <span className="text-xs px-2 py-0.5 bg-primary/5 text-primary rounded border border-primary/10 font-medium">Main</span>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 p-4 min-h-0 bg-background">
        ...
        <div className={cn(
          ...
          viewMode === 'code' ? "md:col-span-9 lg:col-span-6" : "md:col-span-9 lg:col-span-10"
        )}>
          {viewMode === 'code' ? (
            <CodeViewer 
              file={selectedFile} 
